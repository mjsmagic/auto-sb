...
async function getBitBucketEmails() {
  try {
    // get json output of downloaded CSV list
     ...
     const changeStrings = users.map(({ "Public name": name, "Email address": email, ...rest }) => ({name, email, ...rest}));
     //console.log(flat);
...
async function getSlackEmails() { //: Promise<string[]>{
    ...
    let options = {
        ...
        headers: {
            ...
            'Authorization': 'Bearer ACTUAL_SLACK_TOKEN'
        }
    };
    ...
    for (j in (responseProfiles.data.members)){
        ...
        if (rev_display_name != null && rev_email != null) {
           ...
           userList.push("{ name: '" + rev_display_name + "' , email: '" + rev_email + "' }");
